...
BOT_TOKEN  = "8685895736:AAHzICpmNQVrXimEEbyocgdCxQljOpt65dM"
ADMIN_ID   = 6234397895
...
# ==============================
# سيرفر وهمي يمنع Render من النوم
# ==============================
...
async def admin_reply_trigger(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        f"✏️ اكتب ردّك على تذكرة #{tid} وسأرسله للعميل:"
    )
...
async def admin_reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=cid,
        text=f"عذراً، لم نتمكن من معالجة طلبك #{tid} حالياً.\nيمكنك المحاولة مجدداً بكتابة /start"
    )
...
async def show_tickets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for t in reversed(recent):
        lines.append(
            f"#{t['tid']} | {t['status']} | {t['service']}\n"
            f"   👤 {t['fname']} @{t['username']} | 🕐 {t['time']}"
        )
